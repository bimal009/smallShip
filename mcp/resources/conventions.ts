...
5. Run commands through the available sandbox command tools, using the containerId returned by create-sandbox. Run them in /workspace. Use the project's package manager and scripts for dependency installation, migrations, tests, and builds.
   Use pnpm-install with the appId when dependencies need installing, then pnpm-build after making file changes to verify the app builds. Read each tool result's exitCode and output; fix any failures before continuing to the next step.
6. When database changes require it, call pnpm-generate with the appId and verify success, then call pnpm-migrate with the same appId BEFORE running pnpm-build. A build must reflect the current schema, not a stale one. Run the relevant checks and build, inspect their output and exit codes, and fix failures before continuing.
